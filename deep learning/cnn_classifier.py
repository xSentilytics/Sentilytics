...
class TextCNN(nn.Module):
    def __init__(self, vocab_size, num_classes, embedding_matrix=None,
                 ...
                 kernel_sizes=KERNEL_SIZES, emb_dropout=EMB_DROPOUT,
                 dropout=DROPOUT, padding_idx=0):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim, padding_idx=padding_idx)
        if embedding_matrix is not None:
            self.embedding.weight.data.copy_(torch.from_numpy(embedding_matrix))

        self.emb_dropout = nn.Dropout1d(emb_dropout)
    ...
    def forward(self, x):
        ...
        for conv in self.convs:
            ...
            p = F.max_pool1d(c, c.size(2))    # (B, F, 1)
